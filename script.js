document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadState = () => {
        const defaultState = {
            ...
            // Fixed weekly schedule data with START DATES
            recurringSchedules: [],
            customSchedules: [],
        ...
        const saved = localStorage.getItem('englishCourseState');
        if (saved) {
            try {
                ...
                if (!Array.isArray(parsed.recurringSchedules)) parsed.recurringSchedules = [];
                if (!Array.isArray(parsed.customSchedules)) parsed.customSchedules = [];
                if (typeof parsed.cancelledClasses !== 'object') parsed.cancelledClasses = {};
                if (typeof parsed.overrides !== 'object') parsed.overrides = {};
    ...
    const saveStateToStorage = () => {
        localStorage.setItem('englishCourseState', JSON.stringify(state));
    };
